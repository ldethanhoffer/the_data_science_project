...
To simplify our exposition, we first introduce the notation $\bl{\Delta}{u}\define \{\bl{x}{u}\}_{x\in \Delta}\in \d{R}^\Delta$. This set defines the random variable $1_{\bl{\Delta}{u}}$ on $\d{R}$, so that can thus use the language and technology from probability theory.\\
